...
{
	Dio_writePin(SS, STD_LOW);		 //enable slave
	...
	Dio_writePin(SS, STD_HIGH);		//disable slave
...
enuSPi_Status_t Spi_MasterTransmit_array(uint8_t* u8_TxArray, uint16_t u16_arraySize)
{
	...
	{
		Spi_MasterTransmitChar(u8_TxArray[u8_loopCounter]);
	}
	return SPI_STATUS_ERROR_OK;
}
...
enuSPi_Status_t Spi_MasterTransmit_string(uint8_t* u8_TxString)
{
	...
	{
		Spi_MasterTransmitChar(u8_TxString[u8_loopCounter]);
		u8_loopCounter++;
	}
	return SPI_STATUS_ERROR_OK;
}
...
* Parameters (out): Error Status
* Return value: enuSpi_Status_t
* Description: spi slave receives one frame through MOSI pin
******************************************************************************************/
...
enuSPi_Status_t Spi_SlaveReceiveArray(uint8_t* u8_TxData, uint16_t u8_length)
{
	...
	for(u16_loopCounter = Initial_Value; u16_loopCounter < u8_length; u16_loopCounter++)
	{
		while( BIT_IS_CLEAR(SPSR_R, SPIF_B) ) ;					   // While Transmission not completed wait until it's complete
		u8_TxData[u16_loopCounter] =  SPDR_R;                      //Return the received data found in Data register .		
	}
